...
#' Extend `!=` to torch::nn_modules including their initialization parameters.
#' @noRd
`%!=%` <- function(x, y) {
  (is.null(x) && !is.null(y)) ||
    if (is.atomic(x) && is.atomic(y)) {
      ...
      nn_class_delta <- setdiff(union(class(x), class(y)), intersect(class(x), class(y)))
      # different nn_modules || different initialize parameters of the same nn_module
      !is.null(nn_class_delta) || rlang::fn_fmls(x$initialize) %!=% rlang::fn_fmls(y$initialize)
    } else {
      TRUE
    }
...
#' Merge the config= and `...` parameters onto the default `interpretabnet_config()` parameters.
#'
