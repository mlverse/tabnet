...
.make_ix_like <- function(input, dim = 1) {
  d <- input$size(dim)
  rho <- torch_arange(start = 1, end = d, device = input$device,
                             dtype = input$dtype)
...
#' @importFrom torch torch_sort
.threshold_and_support <- function(input, dim) {
  input_srt <- torch_sort(input, descending=TRUE, dim=dim)[[1]]
...
#' @importFrom torch autograd_function torch_clamp torch_where
sparsemax_function <- torch::autograd_function(
  forward = function(ctx, input, dim = -1) {
    ...
    output <- torch_clamp(input - tau_supp_size[[1]], min=0)
  ...
  backward = function(ctx, grad_output) {
    ...
    grad_input <- torch_where(saved$output != 0, grad_input - v_hat, grad_input)
...
#' @importFrom torch torch_clamp torch_sort torch_sqrt
.entmax_threshold_and_support <- function(input, dim) {
  ...
  delta_nz <- torch_clamp(delta, 0)
  tau <- mean - torch_sqrt(delta_nz)
...
#' @importFrom torch autograd_function torch_clamp
entmax_function <- torch::autograd_function(
  forward = function(ctx, input, dim = -1) {
    ...
    output <- torch_clamp(input - tau_supp[[1]], min=0) ^ 2
