...
#   labels <- torch::torch_zeros(c(2, 2), dtype = torch_bool())
#   ancestor <- torch::torch_eye(2)
#   result <- max_constraint_output(output, labels, ancestor)
...
#   output <- torch_tensor(matrix(c(1, 5, 
#                                   3, 2), nrow = 2, ncol = 2, byrow = TRUE))
#   labels <- torch::torch_ones(c(2, 2), dtype = torch_bool())
...
#   output_f32 <- torch_tensor(matrix(1:4, nrow = 2), dtype = torch::torch_float32())
#   output_f64 <- torch_tensor(matrix(1:4, nrow = 2), dtype = torch::torch_float64())
#   labels <- torch::torch_ones(c(2, 2), dtype = torch_bool())
#   ancestor <- torch::torch_eye(2)
...
#   output <- torch_tensor(matrix(1:4, nrow = 2, ncol = 2))
#   labels <- torch::torch_ones(c(2, 3), dtype = torch_bool())
#   ancestor <- torch::torch_eye(2)
...
#   output <- torch_tensor(matrix(1:4, nrow = 2, ncol = 2))
#   labels <- torch::torch_ones(c(2, 2), dtype = torch::torch_float32())
#   ancestor <- torch::torch_eye(2)
...
#                                   FALSE, TRUE), nrow = 2, ncol = 2, byrow = TRUE), dtype = torch::torch_bool())
#   ancestor <- torch_tensor(matrix(c(1, 1, 
#                                     0, 1), nrow = 2, ncol = 2, byrow = TRUE))
#   result <- max_constraint_output(output, labels, ancestor)
