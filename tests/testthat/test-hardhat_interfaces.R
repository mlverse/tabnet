...
test_that("Training multilabel classification for data.frame with validation split", {
  ...
  attriw <- attrix[-which(names(attrix) == "JobSatisfaction")]
  expect_no_error(
    fit <- tabnet_fit(attriw, data.frame(y=attriy, z=attriy, sat=attrix$JobSatisfaction),
...
test_that("Training multilabel mixed output fails with explicit error", {
  ...
  attriw <- attrix[-which(names(attrix) == "PercentSalaryHike")]
  expect_error(
    fit <- tabnet_fit(attriw, data.frame(y = attriy, hik = attrix$PercentSalaryHike), epochs = 1),
