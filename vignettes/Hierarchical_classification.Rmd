...
The tree only keeps a single row of attributes per tree leaf. Thus in order to transfer your complete predictors dataset into the Node object, you must keep the last level of the hierarchy to be a unique sample identifier (last resort beeing `rowid_to_column()` to achieve it).
...
The dataset split here will be done upfront of the `as.Node()` transformation.
We will use `reciepe::initial_split()` to split with a stratification on the parent category of the leaf `_name` which is `homeworld` in our hierarchy.
```{r}
starw_split <- initial_split(starwars, prop = .8, strata = "homeworld")
```
...
```{r model training}
config <- tabnet_config(decision_width = 3, attention_width = 3, num_steps = 3, penalty = .03, cat_emb_dim = 2, valid_split = 0.2, learn_rate = 1e-3, lr_scheduler = "reduce_on_plateau", early_stopping_monitor = "valid_loss", early_stopping_patience = 4, verbose = TRUE)

starw_model <- tabnet_fit(starwars_train_tree, config = config, epoch = 65)

```

## Model diagnostic

Very first diagnostic is the check for model overfitting though the training loss plot
```{r}
...
```{r}
starwars_hat <- predict(starw_model, starwars_test_tree)
starwars_hat
...
We do have local feature importance on the complete dataset here : 
```{r}
...
```
The dataset is a challenge as many new levels are found in many predictors in the testset.
