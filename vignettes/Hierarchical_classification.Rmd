...
You may have notice that `name` and `height` have unexpected values according to the original data: `Human` is not part of the `name` in orginal dataset, and `height` values have been changed into the heigh of the tree. This is due to some rules we will have to follow to create the Node from data frame.

## `Node` preparation rules

### Avoid column name collision with reserved data.tree names

`name` and `height` are both part of the `NODE_RESERVED_NAMES_CONST` reserved list of names for `Node` attribute names. So they must not be used as predictor names.
