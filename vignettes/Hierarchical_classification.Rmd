...
The  `starwars` dataset contains list columns, hosting some variability in the predictor values. So we decide here to `unnest_longer` every list column to each of its values. This triple the size of the `starwars` dataset.  
The dataset split here will be done upfront of the transformation into `as.Node()`.
We will use `rsample::initial_split()` to split with a stratification on the parent category of the leaf `_name` which is `homeworld` in our hierarchy.
```{r}
...
```{r model training}
config <- tabnet_config(decision_width = 8, attention_width = 8, num_steps = 3, penalty = .03, cat_emb_dim = 2, valid_split = 0.2, learn_rate = 1e-3, lr_scheduler = "reduce_on_plateau", early_stopping_monitor = "valid_loss", early_stopping_patience = 4, verbose = TRUE)
...
starwars_hat <- bind_cols(
    ...
    tabnet:::node_to_df(starwars_test_tree)$y
  )
...
The model here is very poor. This is due to the input dataset not collecting large samples of distinctive observation per leaf, but rather a limited number of personages compatible with watching a movie saga.
