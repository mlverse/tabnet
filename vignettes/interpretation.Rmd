...
First let's define some hyperparamaters upfront, including switching our mask from sparsemax to entmax.
```{r}
tab_config <- tabnet_config(epochs = 10, mask_type = "entmax")
```

Now we will pass this configuration and fit our tabnet model.
```{r}
fit_syn2 <- tabnet_fit(y ~ ., syn2, config = tab_config, verbose = TRUE)
```
