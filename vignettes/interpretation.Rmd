...
```{r}
fit_syn2 <- tabnet_fit(y ~ ., syn2, epochs = 45, learn_rate = 0.06, verbose = TRUE, device = "cpu")
```
...
```{r}
#| fig.alt: "3 tabnet explaination plots, one for each step of the fitted model on syn2 dataset. The Step 1 plot shows numerous important observations with V03 and V03 having high importance, step 2 plot highlight the importance of V03 and V06. Third step plot highlight V03 and V10 as important variables"
...
```{r}
fit_syn4 <- tabnet_fit(y ~ ., syn4, epochs = 50, verbose = TRUE, device = "cpu", learn_rate = 0.08)
```
