...
TabNet provides interpretable model as the underlying neural
network architecture uses a feature selection masks that help identify
which features are used at each step.
...
```{r}
fit_syn2 <- tabnet_fit(y ~ ., syn2, epochs = 40, verbose = TRUE)
```
...
We trimmed the color scale to the 99th percentile so the colors shows the
...
```{r}
fit_syn4 <- tabnet_fit(y ~ ., syn4, epochs = 50, verbose = TRUE)
```
...
additional features, depending on `V10`. Step 3 seems to have found some
noise in `V09`, but also focuses strongly on `V01-V02` and `V05-V06`.
