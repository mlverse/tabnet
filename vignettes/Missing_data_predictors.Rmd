...
```{r}
ames_fit <- tabnet_fit(Sale_Price ~ ., data=ames, epoch=40, valid_split = 0.2, verbose=TRUE, batch=2930)
```
...
```{r}
ames_missing_fit <- tabnet_fit(Sale_Price ~ ., data=ames_missing, epoch=40, valid_split = 0.2, verbose=TRUE)
```
