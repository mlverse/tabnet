...
In {tabnet}, we reuse the self-supervised masking mechanism to cover the Missing Data in the training set.

As we enter the world of Missing Data, we have to question the type of Missing Data we deal with. We could have Missing at Random (MAR), like for example some transmission errors on a sensor data dataset, or Missing Not at Random (MNAR) when some interactions exists between the Missing Data and other predictors values for the same sample. The later is a more complex topic to cover, and we will try to investigate it here through the `ames` dataset.
...
We know that it will be extremely difficult for a model to capture an internal representation of such distribution, and thus we want to avoid the null values to bias the model internal representation.
...
Note: A proper value mutation to explicit NAs can also be achieved in a more complete wy: one should check if the qualitative column in the pair `<equipment>_count` - `<equipment>_size` refers to `none` or to zero occurrence of the equipment. But this is beyond the scope of this vignette.

Note: the same dataset `ames_missing` is also present in the package. You can use it through `data("ames_missing", package = "tabnet")`. 
...
cat_emb_dim <- map_dbl(ames %>% select_if(is.factor), ~max(log(nlevels(.x)) %>% floor, 1))

pretrain_config <- tabnet_config(valid_split = 0.2, verbose = TRUE, batch_size = 2930, 
                             cat_emb_dim = cat_emb_dim,
                             early_stopping_patience = 5L,
                             early_stopping_tolerance = 1e-3)


ames_pretrain <- tabnet_pretrain(ames_rec, data = ames,  epoch = 70, 
...
pretrain_config <- tabnet_config(valid_split = 0.2, verbose = TRUE, batch_size = 2930, 
                             cat_emb_dim = cat_emb_dim, learn_rate = 0.04,
                             pretraining_ratio=0.37, early_stopping_patience = 5L,
                             early_stopping_tolerance = 1e-4)

ames_missing_pretrain <- tabnet_pretrain(ames_missing_rec, data = ames_missing, epoch = 70, 
                                    config = pretrain_config)
...
ames_fit <- tabnet_fit(ames_rec, data = ames,  tabnet_model = ames_pretrain, 
                       epoch = 70, config = pretrain_config, learn_rate = 5e-3)
autoplot(ames_fit)
...
```{r}
ames_missing_fit <- tabnet_fit(ames_missing_rec, data = ames_missing, 
                               tabnet_model = ames_missing_pretrain,  epoch = 70,
                               config = pretrain_config, learn_rate = 5e-3
                            )
