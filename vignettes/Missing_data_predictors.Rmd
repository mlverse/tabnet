...
The `ames` dataset from  `{modeldata}`  contains a lot of _null values_ that the human analysis clearly interprete as an implicit _"missing object"_ described by that value. We have for example pool surface of 0 square meters means "no pool", basement surface of 0 square meters means "no basement", ...  
...
Many of those variables come as a pair in the `ames` dataset, one for the qualitative aspect, the other for the quantitative aspect. We have for exemple `Pool_QC` for pool condition, that has a "no_pool" level with `Pool_Area=0` in that case.  
...
Note that this result varies a lot from run to run. The captured result here depends a lot on your initialisation conditions.
...
Even if the models trained here have a huge variability in the variable importance along trainings, we have the intuition that model trained with explicit missing data will provide better result that its counterpart trained with zero-imputed variables.
