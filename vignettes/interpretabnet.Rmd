...
* Experiment scenario  
Let's train 4 different models on ames_missing dataset with two different flavors of parameter, 1 tabnet and 3 interpretabnet.  
...
We will thus have a way to measure the interprestability and compare stability between those 4 models.
...
For the training loop, you will realize that Interpretabnet models requires many more epoch to converge due to the MLP network.

```{r, 3 interpretabnet models }
interpretabnet_config <- interpretabnet_config( cat_emb_dim = cat_emb_dim, verbose = FALSE,
                                early_stopping_patience = 12L, early_stopping_tolerance = 0.01,
                                valid_split = 0.2, learn_rate = 1e-2, lr_scheduler = "step", 
