...
```{r}
collect_metrics <- enframe(mod$fit$metrics,name = "epoch") %>% unnest_longer(value,indices_to = "dataset") %>% unnest_wider(value) %>% mutate(mean_loss = map_dbl(loss, mean))
ggplot(collect_metrics, aes(x=epoch, y=mean_loss, color=dataset)) + geom_line() + scale_y_log10()
```
![pretraining task loss plot](pretraining_loss.png)
## Back to supervised procedure

We reuse our pre-processing steps recipe and feed it directly in a supervised fit pass on top of our pre-trained model. We are going to train for 50 epochs with a batch size of 128. There are other hyperparameters but, we are going to use the defaults.
...
```{r}
collect_fit_metrics <- enframe(pretrained_model_fit$fit$metrics,name = "epoch") %>% unnest_longer(value,indices_to = "dataset") %>% unnest_wider(value) %>% mutate(mean_loss = map_dbl(loss, mean))
ggplot(collect_fit_metrics, aes(x=epoch, y=mean_loss, color=dataset)) + geom_line() + scale_y_log10()
```
...
The question now is What if we did not pretrain the model ? We can build a vanillia tabnet model on the training set for comparison : 

```{r}
vanillia_model_fit <- tabnet_fit(rec_unsup, train , valid_split = 0.2, epochs = 50, verbose=TRUE)
...
```{r}
collect_vanillia_metrics <- enframe(vanillia_model_fit$fit$metrics,name = "epoch") %>% unnest_longer(value,indices_to = "dataset") %>% unnest_wider(value) %>% mutate(mean_loss = map_dbl(loss, mean))
ggplot(collect_vanillia_metrics, aes(x=epoch, y=mean_loss, color=dataset)) + geom_line() + scale_y_log10()
```
...
test %>% 
  bind_cols(
    predict(vanillia_model_fit, test, type = "prob")
  ) %>% 
  roc_auc(Class, .pred_bad)
