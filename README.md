...
fit <- tabnet_fit(rec, train, epochs = 35, valid_split = 0.1, learn_rate = 5e-3, verbose = FALSE)
autoplot(fit)
```
...
``` r
pretrain <- tabnet_pretrain(rec, train, epochs = 70, valid_split=0.1, learn_rate = 1e-2, verbose  = FALSE, )
autoplot(pretrain)
```
